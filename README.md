...
- **Environment driven configuration** - Paths to the database file and listen port are taken from environment variables (`DB_FILE_PATH` and `PORT`). Additional credentials such as the Stripe key, webhook secret and bootstrap API key are loaded from files under a directory specified by `CREDENTIALS_DIRECTORY`.
...
### `/health`
`GET` - Returns `{"status":"unimplemented","db":"unimplemented"}` as a placeholder health check.
...
### `/patrons`
`GET` - Lists patrons. Requires a Bearer token. Supports `limit` and `offset` parameters.
